# turn into arrays, and have a function for printing full text and adding up features of subsets (then add a custom number if counting the header of the subsets)
# Don't count stage directions in characters' speeches
# make a multi text compatible thing
# OPEN GIT PROJECT
    ...
    for actNum in range(len(text['acts'])):
        ...
        for sceneNum in range(len(act['scenes'])):
            ...
            for speechNum in range(len(scene['speeches'])):
                ...
                speech['lines'] = {'text': speakerAndLines[1]}
                speech
